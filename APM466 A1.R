...
f15_18=((1+l1a[5])^5/(1+l1a[4]))^(1/4)-1
...
f15_19=((1+l2a[5])^5/(1+l2a[4]))^(1/4)-1
...
f15_20=((1+l3a[5])^5/(1+l3a[4]))^(1/4)-1
...
f15_21=((1+l4a[5])^5/(1+l4a[4]))^(1/4)-1
...
f15_22=((1+l5a[5])^5/(1+l5a[4]))^(1/4)-1
...
f15_25=((1+l6a[5])^5/(1+l6a[4]))^(1/4)-1
...
f15_26=((1+l7a[5])^5/(1+l7a[4]))^(1/4)-1
...
f15_27=((1+l8a[5])^5/(1+l8a[4]))^(1/4)-1
...
f15_28=((1+l9a[5])^5/(1+l9a[4]))^(1/4)-1
...
f15_29=((1+l10a[5])^5/(1+l10a[4]))^(1/4)-1
...
ytm2=unlist(ytm2)
for (j in 1:9){
  b[j] <- log(ytm2[j+1]/ytm2[j])
}
b

c=vector()
for (j in 1:9){
  c[j] <- log(ytm5[j+1]/ytm5[j])
}
d=vector()
for (j in 1:9){
  d[j] <- log(ytm7[j+1]/ytm7[j])
}
e=vector()
for (j in 1:9){
  e[j] <- log(ytm9[j+1]/ytm9[j])
}
e
f=vector()
for (j in 1:9){
...
d2=vector()
row4=c(0.091871671 , 0.055936187 ,0.050631663,0.05845964,0.054707795 ,0.052374034 , 0.064090295 , 0.056400031,0.086317615 , 0.075952614)
for (j in 1:9){
...
covmatrixfinal_fr
ytmeigen <- eigen(covmatrixfinal_fr)
ytmeigen
